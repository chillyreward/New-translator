...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex text-slate-900 bg-background">
      {/* Left side Form */}
      <div className="flex-1 flex flex-col justify-center px-4 sm:px-6 lg:px-20 xl:px-32 relative z-10">
        <div className="mx-auto w-full max-w-sm lg:w-96">
          <Link href="/" className="flex items-center gap-2 transition-opacity hover:opacity-80 mb-10">
          ...
          <p className="text-slate-500 text-sm mb-8">Start translating with precision and cultural context.</p>

          <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); window.location.href = "/verify-email"; }}>
                    ...
                    {[1, 2, 3, 4].map((level) => (
                      <div 
                        ...
                        className={cn(
                          ...
                          level <= strengthScore ? strengthColor : "bg-slate-100"
                        )}
                      ></div>
                    ))}
                  </div>
                  <div className="grid grid-cols-2 gap-1 gap-x-4">
                    <span className={cn("text-xs flex items-center gap-1", hasLength ? "text-emerald-600" : "text-slate-400")}>
                      <CheckCircle2 size={12} /> 8+ characters
                    </span>
                    <span className={cn("text-xs flex items-center gap-1", hasUpper ? "text-emerald-600" : "text-slate-400")}>
                      <CheckCircle2 size={12} /> Uppercase letter
                    </span>
                    <span className={cn("text-xs flex items-center gap-1", hasNumber ? "text-emerald-600" : "text-slate-400")}>
                      <CheckCircle2 size={12} /> Number
                    </span>
                    <span className={cn("text-xs flex items-center gap-1", hasSpecial ? "text-emerald-600" : "text-slate-400")}>
          ...
          <p className="mt-6 text-center shadow-inner rounded-xl bg-slate-50 py-3 border border-slate-100 text-sm text-slate-500">
      ...
      {/* Right side Illustration Area (hidden on mobile) */}
      <div className="hidden lg:block lg:flex-1 relative overflow-hidden bg-slate-900 border-l border-slate-800">
        ...
        <div className="absolute inset-0 flex items-center justify-center p-20">
          <Card className="w-full max-w-lg bg-white/10 backdrop-blur-xl border-white/10 p-8 shadow-2xl text-white outline outline-1 outline-white/20 outline-offset-8 rounded-3xl">
            ...
            <div className="flex items-center gap-4 mt-8">
              ...
              <div>
                ...
                <p className="text-sm text-white/60">Linguistics Director, Global Reach Org</p>
              </div>
            </div>
          </Card>
        </div>
